...
def paringInputData(input_data):
...
def insertMahjongScore(input_data):
    ...
    # 변수 설정
    wind, name_list, score_list = paringInputData(input_data)
...
def insertUmaScore(input_data):
    ...
    priority_idx = 1
    for idx in range(4):
        result_score[getFullName(name_list[idx])] = [int(int(score_list[idx]) / 1000), priority_idx]
        priority_idx += 1
    ...
    total_uma = 0
    for key in result_score.keys():
        insert_data[getNameIdx(key)] = result_score[key][0]
...
if __name__ == '__main__':
    try:
        ...
        insertUmaScore(test_input_score)
        updateRawData()
        printRank()
